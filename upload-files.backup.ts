  ...
  try {
    ...
    if (fearAndGreedFile) {
      ...
      if (fearAndGreedResult.ok) {
        console.log(
          `Successfully uploaded ${fearAndGreedFile} with main message`
        );
        // Get the timestamp of the message that was just posted
        const history = await client.conversations.history({
          channel: SLACK_CHANNEL_ID,
          limit: 1,
        });
        if (history.ok && history.messages && history.messages.length > 0) {
          initialMessageTs = history.messages[0].ts;
        }
      } else {
        console.error(
          `Failed to upload ${fearAndGreedFile}: ${fearAndGreedResult.error}`
        );
      }
    } else {
